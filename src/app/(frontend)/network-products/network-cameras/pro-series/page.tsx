...
export const metadata = {
  title: 'Pro Series (All) Network Cameras | Hikvision',
  description: 'Professional series network cameras offering high-quality video surveillance solutions',
};

async function fetchProSeriesProducts() {
  try {
    ...
    console.error('Error fetching Pro Series products:', error);
    return [];
  }
}

export default async function ProSeriesPage() {
  const products = await fetchProSeriesProducts();
  ...
  return (
    <div className="bg-white text-black py-12">
      <div className="container mx-auto px-4">
        ...
        {/* Tell ProductFilter not to show its header */}
        <ProductFilter products={products} showHeader={false} />
      </div>
    </div>
  );
}