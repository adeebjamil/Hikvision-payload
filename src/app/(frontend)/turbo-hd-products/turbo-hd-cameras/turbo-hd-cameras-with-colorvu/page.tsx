...
export const metadata = {
  title: 'Turbo HD Cameras with ColorVu | Hikvision UAE',
  description: 'Advanced Turbo HD cameras featuring ColorVu technology for vivid color imaging in low-light conditions',
};

async function fetchTurboHDColorVuProducts() {
  try {
    ...
    const result = await payload.find({
      ...
      where: {
        and: [
          ...
          {
            series: {
              equals: 'turbo-hd-cameras-with-colorvu',
            },
          },
        ],
      },
    ...
    console.error('Error fetching Turbo HD ColorVu products:', error);
    return [];
  }
}

export default async function TurboHDColorVuPage() {
  const products = await fetchTurboHDColorVuProducts();
